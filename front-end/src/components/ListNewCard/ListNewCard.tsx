...
function ListNewCard(newsData) {
  return (
    <>
        {newsData&&newsData.newsData?.data?.data?.map((item: NewsType) => {
            return (<>
                <Link to={`${item.id}`} key={item.id} className='mb-[30px] w-full cursor-pointer px-1 overflow-hidden'>
                    <div className='grid grid-cols-4 gap-4'>
                        <div
                            className='group m-auto col-span-2 relative z-[1] w-full overflow-hidden rounded-lg pt-[56.25%] before:absolute before:inset-0 before:z-[1] before:block before:bg-black before:opacity-0 before:content-[""] hover:before:bg-black hover:before:opacity-[0.2]'
                            style={{
                        ...
                        <div className='col-span-2 m-auto w-full'>
                            <h4 className='text-mainGreenColor inline flex items-center rounded-lg'><span className='mr-2'><NewspaperIcon/></span> Tin tức</h4>
                            <h5 className='overflow-hiden mt-[5px]  text-[18px] font-semibold leading-snug text-[#292929] break-words'>{item.attributes.title}</h5>
                            <p className='mt-[25px] text-[16px] leading-snug line-clamp-2 break-words'>{item.attributes.description}</p>
